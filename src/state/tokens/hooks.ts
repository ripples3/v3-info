    ...
    useEffect(() => {
        async function fetch() {
            ...
            if (!loading && !error && addresses) {
                dispatch(
                    addPoolAddresses({ tokenAddress: address, poolAddresses: addresses, networkId: activeNetwork.id }),
                );
            }
            if (error) {
                setError(error);
    ...
    useEffect(() => {
        async function fetch() {
            ...
            if (error) {
                ...
                dispatch(
                    updateTransactions({ tokenAddress: address, transactions: data, networkId: activeNetwork.id }),
                );
            }
        }
